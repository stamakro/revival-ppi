	...
	for i in range(F + 1):
		...
		if i == 0:
			...
			for j,p in enumerate(combinations(range(F), i)):
				...
				if classifier == 'gba' and os.path.exists(experimentPath + 'fold' + str(fold_nr) + '/ppi/' + str(counter) + '_knn_weightedString.pkl'):
					continue
				print('Fold:%d, Iteration %d/%d' % (fold, counter, ss), flush=True)
				...
				if classifier == 'gba':
					...
					with open(experimentPath + 'fold' + str(fold_nr) + '/ppi/' + str(counter) + '_knn_weightedString.pkl', 'wb') as fw:
						pickle.dump(pp, fw)
'''
