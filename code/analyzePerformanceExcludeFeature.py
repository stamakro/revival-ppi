...
i1 = np.where(datasources == 'textmining')[0]
i2 = np.where(datasources == 'textmining_transferred')[0]
ii = [i1[0], i2[0]]

d2i = ds2ind(datasources)

noTmHom = np.where(np.sum(d2i[ii],0) == 0)[0]

subFm = meanMatrix[0, noTmHom]
subFs = stdMatrix[0, noTmHom]

subSm = meanMatrix[2, noTmHom]
subSs = stdMatrix[2, noTmHom]

print('tm')
bestF = np.argmax(subFm)
assert bestF > 0
print(bestF)
print(subFm[bestF], subFs[bestF])

bestS = np.argmin(subSm)
print(bestS)
...
i1 = np.where(datasources == 'homology')[0]

d2i = ds2ind(datasources)

noTmHom = np.where(d2i[i1] == 0)[1]
...
i1 = np.where(datasources == 'textmining')[0]
i2 = np.where(datasources == 'homology')[0]
i3 = np.where(datasources == 'textmining_transferred')[0]
ii = [i1[0], i2[0], i3[0]]
...
fig.savefig('../figures/clustering/per_data_source/' + species + '_fmax.png')
...
fig.savefig('../figures/clustering/per_data_source/' + species + '_smin.png')
